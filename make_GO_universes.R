...
##  legacy  ##
# 
# ##  MAKE AN EXCEL FILE WITH ALL OF THE BLAST BASED GO ANNOTATIONS (this was then manually curated and multicellular GO terms were identified from this table)  ##---------------------------------------------------------------------------------------------------------------------------------------------
# # combine all BLAST results
# all_BLAST_based_annotations <- rbind(arabidopsis_BLAST_derived_GO_universe[, c(1:3)], plasmodium_BLAST_derived_GO_universe[, c(1:3)])
# all_BLAST_based_annotations <- rbind(all_BLAST_based_annotations, drosophila_BLAST_derived_GO_universe[, c(1:3)])  
# 
# # make a list of the GO terms to look at, those specifcic to the BLAST results
# all_other_organisms_BLAST_based_annotations_unique_GO_terms <- unique(all_BLAST_based_annotations$frame.go_id)
# tetrahymena_annotations_unique_GO_terms <- unique(tet_all_annotations_df$frame.go_id)
# all_other_organisms_BLAST_based_annotations_unique_GO_terms_not_in_Tet <- all_other_organisms_BLAST_based_annotations_unique_GO_terms[! all_other_organisms_BLAST_based_annotations_unique_GO_terms %in% tetrahymena_annotations_unique_GO_terms]
# 
# 
# ## false to parse::
# dos <- drosophila_BLAST_derived_GO_universe[, c(1:3)]
# unique_GO_terms <- unique(dos$frame.go_id)
# 
# all_others <- rbind(ciliate_org_GO_universe, tetramine_GO_universe)
# all_others <- rbind(all_others, arabidopsis_BLAST_derived_GO_universe[,c(1:3)])
# all_others <- rbind(all_others, geneOntology_org_GO_universe)
# all_others <- rbind(all_others, plasmodium_BLAST_derived_GO_universe[,c(1:3)])
# all_other_unique_GO_terms <- unique(all_others$frame.go_id)
# 
# all_other_organisms_BLAST_based_annotations_unique_GO_terms_not_in_Tet <- unique_GO_terms[! unique_GO_terms %in% all_other_unique_GO_terms]
# 
# all_other_organisms_BLAST_based_annotations_unique_GO_terms_not_in_Tet <- all_other_organisms_BLAST_based_annotations_unique_GO_terms_not_in_Tet[! all_other_organisms_BLAST_based_annotations_unique_GO_terms_not_in_Tet %in% GO_terms_to_exclude]
# 
# length(all_other_organisms_BLAST_based_annotations_unique_GO_terms_not_in_Tet)
# #initilize and empty df
# output_df_to_curate <- data.frame(GO_id = as.character(), GO_term = as.character(), GO_definition = as.character(), GO_ontology_type = as.character())
# 
# # for every BLAST based GO terms add the name, definition, and ontology type to a row and add it to the initilized df above
# for (GO_term_to_curate in all_other_organisms_BLAST_based_annotations_unique_GO_terms_not_in_Tet){
#   term <- Term(GO_term_to_curate)[[1]]  # add name
#   Definition <- Definition(GO_term_to_curate)[[1]]  # add definition
#   Ontology <- Ontology(GO_term_to_curate)[[1]]  # add the ontology type
#   
#   df_row_to_add <- data.frame(GO_id = GO_term_to_curate, GO_term = term, GO_definition = Definition, GO_ontology_type = Ontology)
#   output_df_to_curate <- rbind(output_df_to_curate, df_row_to_add)
# }
# # write.xlsx2(output_df_to_curate,file="BLASTp_derived_GO_annotations_definitions_nonTetGOterms_20180910.xlsx",sheetName = "sheet1", row.names = F)
# write.xlsx2(output_df_to_curate,file="BLASTp_drosophila_derived_GO_annotations_definitions_nonTetGOterms_20180925.xlsx",sheetName = "sheet1", row.names = F)
# 